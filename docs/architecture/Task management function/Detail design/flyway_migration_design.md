...
| テーブル作成 | V2__create_user_tables.sql | ユーザー系テーブル作成 |
| テーブル作成 | V3__create_project_tables.sql | プロジェクト系テーブル作成 |
| テーブル作成 | V4__create_task_tables.sql | タスク系テーブル作成 |
| テーブル作成 | V5__create_history_tables.sql | 招待・パスワードリセット系テーブル作成 |
| 初期データ | V6__insert_master_data.sql | 固定マスタ初期データ投入 |
| 開発用初期データ | V7__insert_dev_initial_admin.sql | 開発環境用の初期管理者投入。共通マイグレーションには含めない |
...
- 開発用初期管理者データの投入方法
- 期限切れ招待情報を自動更新するか、参照時に判定するか
- 期限切れパスワードリセット情報を自動更新するか、参照時に判定するか